...
from util.db_util import read_correspond_answer_from_java_table
from util.str_util import split_into_paragraph
import time
import operator
from _2_SentenceSelection.Order.Order_Analysis import get_order_score
from _2_SentenceSelection.Pattern.Pattern_Analysis import get_pattern_score
from _2_SentenceSelection.HTMLTag.HTML_Analysis import get_html_score
from _2_SentenceSelection.Entropy.Entropy_Analysis import get_entropy_score
from _2_SentenceSelection.Entity.Entity_Analysis import get_entity_score, get_entities_from_text
...
import sys
from util.experiment_util import save_ss_result_our_approach
from pathutil import get_base_path
from MMR.MMR_Analysis import MMR_Analysis
from util.experiment_util import load_Step1_result
...
if __name__ == '__main__':
    ...
    name_of_Step1_approach = 'my_approach'
    query_top_dq = load_Step1_result(name_of_Step1_approach, top_relevant_question_num)
    dir_of_ss_result = get_base_path() + '/_2_Result/Approach_MMR_All/'
    # parameter
    top_relevant_paragraph_num = 5
    for i in range(0, len(query_top_dq), 1):
        print 'processing No.' + str(i) + ' query', time.strftime('%Y-%m-%d %H:%M:%S')
        query = query_top_dq[i][0]
        top_dq = query_top_dq[i][1]
        # print 'query : ' + query
        ...
        path_of_ss_result = dir_of_ss_result + str(i) + '.txt'
        save_ss_result_our_approach(query, top_ss, summary, path_of_ss_result)
